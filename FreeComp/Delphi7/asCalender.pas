...
uses StdCtrls, ExtCtrls, Controls, Classes, Grids, Types, Graphics, Windows, Messages, Forms, SysUtils;
...
type
  ...
  TCellMouseDownEvent = procedure (Sender: TObject; ACol, ARow, X, Y: Longint;
    Canvas: TCanvas; aRect: TRect; CellDate: TDate; Enabled: Boolean; Button: TMouseButton) of object;
  ...
  private
	...
	TasCalender = class(TCustomDrawGrid)
  private
    SelectedCol: Integer;
    SelectedRow: Integer;
...
begin
  ...
  begin
    ...
    if IsDoCellMouseDownStored then
      FDoCellMouseDown(Self, ColPressed, RowPressed, Message.XPos, Message.YPos, Canvas, aRect, aDate, isEnabled, mbLeft);
  end;
end;
...
begin
  ...
  begin
    ...
    if IsDoCellMouseUpStored then
      FDoCellMouseUp(Self, ACol, ARow, Message.XPos, Message.YPos, Canvas, aRect, aDate, isEnabled, mbLeft);
  end;
end;

procedure TasCalender.WMRButtonDown(var Message: TWMRButtonDown);
var
  aRect: TRect;
  isEnabled: Boolean;
  aDate: TDate;
begin
  ...
  begin
    ...
    aDate := DateofFirstCell + 7*(RowPressed-1)+ColPressed;
    if IsDoCellMouseDownStored then
      FDoCellMouseDown(Self, ColPressed, RowPressed, Message.XPos, Message.YPos, Canvas, aRect, aDate, isEnabled, mbRight);
  end;
end;

procedure TasCalender.WMRBUTTONUP(var Message: TWMRBUTTONUP);
var
  ACol,ARow: Integer;
  aRect: TRect;
  isEnabled: Boolean;
...
begin
  ...
  begin
    ...
    if IsDoCellMouseUpStored then
      FDoCellMouseUp(Self, ACol, ARow, Message.XPos, Message.YPos, Canvas, aRect, aDate, isEnabled, mbRight);
  end;
end;

...
{$ENDIF}
  ...
  published
    ...
    property LeftPopup: Boolean read FLeftPopup write FLeftPopup
      default False;
    property StartMinimized: Boolean read FStartMinimized write
FStartMinimized
      default False;         // Main form minimized on app. start-up?
    property MinimizeToTray: Boolean read FMinimizeToTray write
FMinimizeToTray
...
constructor TasTray.Create(AOwner: TComponent);
begin
  ...
  IconData.wnd := Classes.AllocateHWnd(HandleIconMessage);
...
destructor TasTray.Destroy;
begin
  SetIconVisible(False);     // Remove the icon from the tray
  FIcon.Free;                // Free the icon
  Classes.DeallocateHWnd(IconData.Wnd);   // Free the tray window
  CycleTimer.Free;
...
procedure TasTray.Notification(AComponent: TComponent;
...
{ For MinimizeToTray to work, we need to know when the form is minimized
  ...
  The straight-forward way is to make TasTray trap the
  Application.OnMinimize event. However, if you also make use of this
  event in the application, the OnMinimize code used by TasTray
  is discarded.
...
procedure TasTray.HookAppProc(var Msg: TMessage);
begin
  case Msg.Msg of
    WM_SIZE:
...
procedure TasTray.UnhookParent;
begin
...
procedure TasTray.HookWndProc(var Msg: TMessage);
begin
  case Msg.Msg of
...
procedure TasTray.HandleIconMessage(var Msg: TMessage);
...
begin
  ...
  begin
    ...
    case Msg.Msg of
      ...
      Msg.Result := DefWindowProc(IconData.Wnd, Msg.Msg, Msg.wParam,
Msg.lParam);
    end;
end;


procedure TasTray.SetIcon(Value: TIcon);
begin
  FIcon.Assign(Value);
  ModifyIcon;
end;
...
procedure TasTray.SetIconVisible(Value: Boolean);
begin
  if Value then
    ShowIcon
  else
...
procedure TasTray.SetDesignPreview(Value: Boolean);
begin
...
procedure TasTray.SetCycleIcons(Value: Boolean);
begin
  FCycleIcons := Value;
  if Value then
    SetIconIndex(0);
...
procedure TasTray.SetCycleInterval(Value: Cardinal);
begin
...
procedure TasTray.SetIconList(Value: TImageList);
begin
  FIconList := Value;
{
...
procedure TasTray.SetIconIndex(Value: Integer);
begin
...
procedure TasTray.SetHint(Value: String);
begin
  FHint := Value;
  ModifyIcon;
end;
...
procedure TasTray.SetShowHint(Value: Boolean);
begin
  FShowHint := Value;
  ModifyIcon;
end;
...
function TasTray.InitIcon: Boolean;
...
function TasTray.ShowIcon: Boolean;
...
function TasTray.HideIcon: Boolean;
...
function TasTray.ModifyIcon: Boolean;
...
procedure TasTray.TimerCycle(Sender: TObject);
begin
...
procedure TasTray.ShowMainForm;
begin
...
procedure TasTray.HideMainForm;
begin
...
procedure TasTray.PopupAtCursor;
var
  CursorPos: TPoint;
begin
...
//        if not (csDesigning in ComponentState) then
          if Screen.ActiveControl <> nil then
            if (Screen.ActiveControl.Owner is TWinControl) then
              SetForegroundWindow((Screen.ActiveControl.Owner as
TWinControl).Handle);
...
procedure TasTray.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  // Execute user-assigned method
...
procedure TasTray.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  // Execute user-assigned method
...
procedure TasTray.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
  // Execute user-assigned method
...
procedure TasTray.DoMinimizeToTray;
begin
...
procedure Register;
begin
  RegisterComponents('GVS', [TasTray]);
end;

...
type
  ...
  private
    ...
    procedure PaintText;
    procedure DoDrawText(var Rect: TRect; Flags: Longint; aColor: TColor); dynamic;
...
procedure TasSpeedButton.DoDrawText(var Rect: TRect; Flags: Integer; aColor: TColor);
var
  Text: string;
      ...
      begin
        ...
        if RaiseSelectedImage then
          ...
          begin
            ...
            try
              aBitMap.Canvas.brush.Color := $000008;
              aBitMap.Canvas.fillRect(Rect(0,0,aBitMap.Width,aBitMap.height));
              Images.GetBitmap(ImageIndex,aBitMap);
              X := 0;
              While X < Images.Width do
              begin
                Y := 0;
                While Y < Images.Height do
                begin
                  if aBitMap.Canvas.Pixels[X,Y] <> $000008 then
                  Canvas.Pixels[aLeft+2+x,aTop+2+y] := clBtnShadow;
                  Y := Y +1;
                end;
                X := X +1;
              end;
        ...
        {$ENDIF}
        Images.Draw(Canvas,aLeft,aTop,ImageIndex);
      end;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    DoDrawText(CalcRect, DrawStyle or DT_CALCRECT, aColor);
  ...
  if UseNewRect then
    DoDrawText(NewRect,DrawStyle,aColor)
  else
    DoDrawText(Rect,DrawStyle,aColor);
end;
